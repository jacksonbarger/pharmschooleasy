...
function GlassmorphismPharmacyApp() {
  ...
  const sidebarItems = [
    { id: 'dashboard', label: 'Dashboard', icon: BarChart3, color: 'from-cyan-400 to-blue-500', progress: 85 },
    { id: 'liver', label: 'Liver System', icon: '🫁', color: 'from-emerald-400 to-teal-500', progress: 75 },
    { id: 'cardio', label: 'Cardiovascular', icon: Heart, color: 'from-rose-400 to-pink-500', progress: 60 },
    { id: 'renal', label: 'Renal System', icon: '🫘', color: 'from-amber-400 to-orange-500', progress: 0 },
              ...
              {sidebarItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-0 group-hover:opacity-10 transition-all duration-300`}></div>
                    ...
                    <div className="relative p-4 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <span className={`font-semibold ${isActive ? 'text-white' : 'text-white/80'}`}>
                          {item.label}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                        {item.progress !== undefined && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-white/70 font-medium min-w-[3ch]">{item.progress}%</span>
                          </div>
                        )}
                        <ChevronRight className={`h-5 w-5 text-white/40 transition-transform duration-300 ${isActive ? 'rotate-90 text-white/80' : 'group-hover:translate-x-1'}`} />
                      </div>
                    </div>
                  </button>
                )
            ...
            <div className="p-8">
                        ...
                        <div className="flex gap-3">
                          ...
                          <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-4 max-w-md border border-white/20">
                            <p className="text-white">
                              👋 Hello! I'm your AI pharmacy tutor. I can help you with drug mechanisms, 
                              clinical applications, side effects, interactions, and more. What would you like to learn about today?
                            </p>
                          </div>
                        </div>
                      </div>
              ...
              {activeTab === 'dashboard' && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {[
                    { title: 'Liver System', progress: 75, color: 'from-emerald-400 to-teal-500', icon: '🫁' },
                    { title: 'Cardiovascular', progress: 60, color: 'from-rose-400 to-pink-500', icon: '❤️' },
                    { title: 'Renal System', progress: 0, color: 'from-amber-400 to-orange-500', icon: '🫘' }
                  ].map((module, index) => (
                    <div key={index} className="group relative overflow-hidden bg-white/10 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl hover:shadow-3xl transition-all duration-500 hover:scale-105">
                      <div className={`absolute inset-0 bg-gradient-to-br ${module.color} opacity-5 group-hover:opacity-10 transition-all duration-300`}></div>
                      <div className="relative p-8">
                        <div className="flex items-center justify-between mb-6">
                          <div className={`p-4 bg-gradient-to-r ${module.color} rounded-2xl shadow-lg`}>
                ...
                <div className="bg-white/10 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl p-12 text-center">
                  <div className="mb-6">
                    ...
                    <p className="text-white/70 text-lg">
                      This module is under development. Check back soon for comprehensive {activeTab} content!
                    </p>
                  </div>
                </div>
              )}
