...
export const SecureApiKeyConfig: React.FC<SecureApiKeyConfigProps> = ({
  ...
  const [validationStatus, setValidationStatus] = useState<'idle' | 'valid' | 'invalid' | 'error'>(
    'idle'
  );
  ...
  const handleClearKey = () => {
    if (
      window.confirm('Are you sure you want to remove your API key? This will disable AI features.')
    ) {
    ...
    <div className={`glass-purple rounded-2xl p-6 ${className}`}>
      ...
      <div className='space-y-4'>
        ...
        <div className='space-y-3'>
          <label className='block text-white/80 font-medium'>OpenAI API Key</label>
