...
import { Brain, Clock, Target, CheckCircle, XCircle, Play, Pause, SkipForward, SkipBack, Trophy, BookOpen } from 'lucide-react'
          ...
          <div className="mt-6">
            <div className="w-full bg-white/20 rounded-2xl h-3 overflow-hidden">
              <div
                ...
                style={{ '--progress-width': `${((currentQuestionIndex + 1) / currentSession.totalQuestions) * 100}%` } as React.CSSProperties}
              />
            </div>
          </div>
        </CardContent>
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl ${currentQuestion.type === 'multiple-choice' ? 'bg-blue-100 text-blue-600' :
                      currentQuestion.type === 'true-false' ? 'bg-green-100 text-green-600' :
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-500">
                      Estimated time: {Math.floor(currentQuestion.estimatedTime / 60)}:{(currentQuestion.estimatedTime % 60).toString().padStart(2, '0')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {isAnswered && (
                <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl ${currentAttempt?.isCorrect
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-2">
                    ...
                    {currentQuestion.references && currentQuestion.references.length > 0 && (
                      <div className="mt-3">
                        <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">Related Drugs:</p>
                        <div className="flex flex-wrap gap-2">
                          {currentQuestion.references.map((ref, index) => (
                            <span key={index} className="px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-lg text-xs font-medium">
                              {ref}
                            </span>
                          ))}
                        </div>
...
function QuestionAnswerArea({ question, selectedAnswer, onAnswerChange, disabled, showCorrect }: QuestionAnswerAreaProps) {
      ...
      <div className="space-y-3">
        {question.options?.map((option: string, index: number) => {
          ...
          return (
            <label
              key={index}
              className={`block p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 ${disabled ? 'cursor-not-allowed opacity-75' :
                  isSelected ? 'border-purple-400 bg-purple-50 dark:bg-purple-900/30' :
                    'border-gray-200 dark:border-gray-700 hover:border-purple-300 hover:bg-purple-50/50 dark:hover:bg-purple-900/10'
                } ${showCorrect && isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-900/30' :
                  showCorrect && isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-900/30' : ''
                }`}
            >
      ...
      <div className="grid grid-cols-2 gap-4">
        {['True', 'False'].map((option) => {
          ...
          return (
            <label
              key={option}
              className={`block p-6 rounded-2xl border-2 cursor-pointer transition-all duration-200 text-center ${disabled ? 'cursor-not-allowed opacity-75' :
                  isSelected ? 'border-purple-400 bg-purple-50 dark:bg-purple-900/30' :
                    'border-gray-200 dark:border-gray-700 hover:border-purple-300 hover:bg-purple-50/50 dark:hover:bg-purple-900/10'
                } ${showCorrect && isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-900/30' :
                  showCorrect && isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-900/30' : ''
            ...
            >
              <div className="space-y-3">
                <div className={`w-8 h-8 rounded-full border-2 mx-auto flex items-center justify-center ${isSelected ? 'border-purple-500 bg-purple-500' : 'border-gray-300 dark:border-gray-600'
                  } ${showCorrect && isCorrect ? 'border-green-500 bg-green-500' :
                    showCorrect && isSelected && !isCorrect ? 'border-red-500 bg-red-500' : ''
                  }`}>
                  {(isSelected || (showCorrect && isCorrect)) && (
      ...
      <div className="space-y-4">
        ...
        {showCorrect && (
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-2xl">
            <p className="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2">Sample Answer:</p>
